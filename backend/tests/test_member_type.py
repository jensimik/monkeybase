...
def test_get_member_types(client: TestClient):
    response = client.get("/member-types")
...
def test_get_members_of_member_type(auth_client_admin: TestClient, member_type_id=1):
    response = auth_client_admin.get(f"/member-types/{member_type_id}/members")
    assert response.status_code == status.HTTP_200_OK
...
def test_get_member_type(client: TestClient, member_type_id=1):
    response = client.get(f"/member-types/{member_type_id}")
    assert response.status_code == status.HTTP_200_OK
...
def test_patch_member_type(auth_client_admin: TestClient, fake_name: str):
    response = auth_client_admin.patch("/member-types/1", json={"name": fake_name})
    assert response.status_code == status.HTTP_200_OK
    ...
    # unknown id
    response = auth_client_admin.patch("/member-types/999999", json={"name": fake_name})
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_delete_member_type(auth_client_admin: TestClient):
    ...
    response = auth_client_admin.delete(f"/member-types/{id}")

    assert response.status_code == status.HTTP_204_NO_CONTENT

    response = auth_client_admin.get(f"/member-types/{id}")

    assert response.status_code == status.HTTP_404_NOT_FOUND

    # remove a member_type with members

    response = auth_client_admin.delete(f"/member-types/1")
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_reserve_a_slot(auth_client_basic: TestClient):
    response = auth_client_basic.post("/member-types/1/reserve-a-slot")
    assert response.status_code == status.HTTP_200_OK
...
def test_no_access(c: TestClient):
    for response in [
        c.patch("/member-types/1", json={"name": "new name"}),
        c.post("/member-types", json={"name": "new user"}),
        c.delete("/member-types/1"),
    ]:
